...
export default function AccountSearchForm({ defaultValue = "" }: Props) {
  ...
  function handleSubmit(e: FormEvent) {
    ...
    if (!q) return;
    startTransition(() => {
      router.push(`/threads/analyze?account=${encodeURIComponent(q)}`);
    });
  }
    ...
    <form onSubmit={handleSubmit} className="w-full max-w-xl mx-auto">
      <div className="flex items-center gap-2 bg-gray-900 border border-gray-700 rounded-xl px-4 py-3">
        ...
        <input
          ...
          placeholder="username 또는 Threads URL"
