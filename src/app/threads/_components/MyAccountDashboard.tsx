    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4">
        ...
        <div className="bg-gray-900 border border-gray-800 rounded-xl p-5">
          <h3 className="text-xs text-gray-500 mb-3 font-medium flex items-center gap-1.5">
            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="text-teal-500"><circle cx="12" cy="7" r="4"/><path d="M5.5 21a6.5 6.5 0 0 1 13 0"/></svg>
            프로필 개요
          </h3>
          <div className="space-y-2">
            <div className="flex justify-between items-baseline">
              <span className="text-xs text-gray-500">팔로워</span>
              <span className="text-lg font-bold text-white">{fmt(summary.followers_count)}</span>
            </div>
            <div className="flex justify-between items-baseline">
              <span className="text-xs text-gray-500">조회수</span>
              <span className="text-lg font-bold text-white">{fmt(summary.views)}</span>
            </div>
            <div className="flex justify-between items-baseline">
              <span className="text-xs text-gray-500">좋아요</span>
              <span className="text-lg font-bold text-white">{fmt(summary.likes)}</span>
            </div>
            <div className="grid grid-cols-3 gap-2 mt-2 pt-2 border-t border-gray-800">
              <div className="text-center">
                <p className="text-sm font-bold text-white">{fmt(summary.replies)}</p>
                <p className="text-[10px] text-gray-600">댓글</p>
              </div>
              <div className="text-center">
                <p className="text-sm font-bold text-white">{fmt(summary.reposts)}</p>
                <p className="text-[10px] text-gray-600">리포스트</p>
              </div>
              <div className="text-center">
                <p className="text-sm font-bold text-white">{fmt(summary.quotes)}</p>
                <p className="text-[10px] text-gray-600">인용</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="bg-gray-900 border border-gray-800 rounded-xl p-5">
          <h3 className="text-xs text-gray-500 mb-3 font-medium flex items-center gap-1.5">
            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="text-orange-400"><rect x="3" y="4" width="18" height="18" rx="2" ry="2"/><line x1="16" y1="2" x2="16" y2="6"/><line x1="8" y1="2" x2="8" y2="6"/><line x1="3" y1="10" x2="21" y2="10"/></svg>
            게시 연속 기록
          </h3>
          <div className="text-center mb-3">
            <span className="text-3xl font-bold text-white">{dashboard.streak}</span>
            <span className="text-sm text-gray-500 ml-1">일 연속</span>
          </div>
          {/* 최근 30일 히트맵 */}
          ...
          <p className="text-[10px] text-gray-600 mt-2">최근 30일</p>
        </div>

        {/* 3. 인기 콘텐츠 */}
        <div className="bg-gray-900 border border-gray-800 rounded-xl p-5">
          <h3 className="text-xs text-gray-500 mb-3 font-medium flex items-center gap-1.5">
            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="text-yellow-400"><line x1="18" y1="20" x2="18" y2="10"/><line x1="12" y1="20" x2="12" y2="4"/><line x1="6" y1="20" x2="6" y2="14"/></svg>
            인기 콘텐츠
          </h3>
          <div className="space-y-2">
            {dashboard.topPosts.length === 0 && (
              <p className="text-xs text-gray-600">게시물이 없습니다</p>
            )}
            {dashboard.topPosts.map((p, i) => (
              <a
                ...
                className="block hover:bg-gray-800/50 rounded-lg p-2 -mx-2 transition"
              >
        ...
        <div className="bg-gray-900 border border-gray-800 rounded-xl p-5">
          ...
          {dashboard.topTopics.length === 0 ? (
            <p className="text-xs text-gray-600">아직 데이터가 부족합니다</p>
          ) : (
            <div className="space-y-2">
              ...
              {dashboard.topTopics.map((t, i) => (
                <div key={i} className="bg-gray-800/50 rounded-lg p-2">
                  <p className="text-xs text-gray-300 line-clamp-2">{t.topic}...</p>
      ...
      <div className="bg-gray-900 border border-gray-800 rounded-xl p-6">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
          ...
          <div>
            <p className="text-2xl font-bold text-white">{fmt(averages.avg_views)}</p>
